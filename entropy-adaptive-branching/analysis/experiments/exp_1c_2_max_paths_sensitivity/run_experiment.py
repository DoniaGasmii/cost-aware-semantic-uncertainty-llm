"""
Run Experiment 1.C.2: Max Paths Sensitivity

Variable: max_paths
Fixed: threshold, branch_factor, temperature, prompt_length
"""
...
def run_single_experiment(
    ...
    """Run experiment with specific max_paths."""
    prompt_text = prompt['text']

    print(f"  Prompt: {prompt['id']} | Max paths: {max_paths}")

    # Initialize EAB with specific max_paths
    print(f"    Initializing EAB with max_paths={max_paths}...")
    device = config['model']['device']
    eab = EntropyAdaptiveBranching(
        model_name=config['model']['name'],
        entropy_threshold=config['eab']['entropy_threshold'],
        branch_factor=config['eab']['branch_factor'],
        max_paths=max_paths,  # VARIABLE
        device=device,
...
def main():
    print("=" * 70)
    print("EXPERIMENT 1.C.2: MAX PATHS SENSITIVITY")
    print("=" * 70)
    ...
    if config['debug']['enabled']:
        print("   🔧 DEBUG MODE")
        max_paths_values = config['debug']['max_paths_values']
        prompts_per_value = config['debug']['prompts_per_value']
    else:
        print("   🚀 FULL EXPERIMENT MODE")
        max_paths_values = config['max_paths_values']
        prompts_per_value = config['prompts_per_value']

    print(f"   Max paths values: {max_paths_values}")
    print(f"   Prompts per value: {prompts_per_value}")
    ...
    save_json({
        'metadata': {
            'config': config,
            'total_experiments': len(max_paths_values) * prompts_per_value
        },
        'results': []
    }, results_file)
    ...
    prompts = load_prompts(prompts_dir)[:prompts_per_value]
    ...
    all_results = []
    total_experiments = len(max_paths_values) * len(prompts)
    experiment_count = 0

    for mp in max_paths_values:
        print(f"\n🛤️  Max Paths: {mp}")
        print("-" * 70)
        ...
        for prompt in prompts:
            ...
            try:
                result = run_single_experiment(prompt, mp, model, tokenizer, config)
