"""
Run Experiment 1.C.1: Branch Factor Sensitivity

Variable: branch_factor
Fixed: threshold, max_paths, temperature, prompt_length
"""
...
def compute_quality_metrics(samples: List[Dict]) -> Dict[str, float]:
    """Compute basic quality metrics from samples."""
    if not samples:
        return {}
    ...
    # TODO: Add diversity_score (Self-BLEU) and coherence_score (perplexity)
    return {
...
def run_single_experiment(
    ...
    """Run experiment with specific branch_factor."""
    prompt_text = prompt['text']

    print(f"  Prompt: {prompt['id']} | Branch factor: {branch_factor}")

    # Initialize EAB with specific branch_factor
    print(f"    Initializing EAB with branch_factor={branch_factor}...")
    device = config['model']['device']
    eab = EntropyAdaptiveBranching(
        model_name=config['model']['name'],
        entropy_threshold=config['eab']['entropy_threshold'],
        branch_factor=branch_factor,  # VARIABLE
    ...
    eab_samples, eab_metrics = run_eab_generation(prompt_text, eab, config, eab_tracker)
...
def main():
    print("=" * 70)
    print("EXPERIMENT 1.C.1: BRANCH FACTOR SENSITIVITY")
    print("=" * 70)
    ...
    if config['debug']['enabled']:
        print("   🔧 DEBUG MODE")
        branch_factors = config['debug']['branch_factors']
        prompts_per_factor = config['debug']['prompts_per_factor']
    else:
        print("   🚀 FULL EXPERIMENT MODE")
        branch_factors = config['branch_factors']
        prompts_per_factor = config['prompts_per_factor']

    print(f"   Branch factors: {branch_factors}")
    print(f"   Prompts per factor: {prompts_per_factor}")
    print(f"   Device: {config['model']['device']}")

    # Load model (shared across experiments)
    ...
    save_json({
        'metadata': {
            'config': config,
            'total_experiments': len(branch_factors) * prompts_per_factor
        },
        'results': []
    }, results_file)
    ...
    prompts = load_prompts(prompts_dir)[:prompts_per_factor]
    ...
    all_results = []
    total_experiments = len(branch_factors) * len(prompts)
    experiment_count = 0
    ...
    for bf in branch_factors:
        ...
        for prompt in prompts:
            ...
            try:
                result = run_single_experiment(prompt, bf, model, tokenizer, config)
